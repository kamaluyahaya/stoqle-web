...
export default function PaymentInfoModal({ open, prefKey, initialValue, onClose, onSave, role = "vendor" }: Props) {
  ...
  async function handleSave() {
    ...
    const payloadJson = JSON.stringify({
      ...payloadObj,
      // Legacy compatibility for local storage keys
    ...
    try {
      ...
      // 2. Direct Backend Save for Users (Automated Recipient Generation)
      if (role === 'user') {
        try {
          await savePaymentAccount(payloadObj);
          console.debug("Payment account registered with StoqlePay (Paystack)");
        } catch (apiErr: any) {
          console.error("Failed to sync payment info with backend:", apiErr);
          toast("Saved locally, but server sync failed. Please check your internet.");
        }
      }
      ...
      toast("Payment info saved successfully.");
    } catch (e) {
      ...
      {open && (
        <div className="fixed inset-0 z-[1001] flex items-end sm:items-center justify-center p-4" role="dialog" aria-modal="true">
          ...
          <motion.div
            ...
            className="relative w-full max-w-2xl bg-white lg:rounded-2xl rounded-t-2xl shadow-xl p-4 z-10 flex flex-col"
          >
              ...
              {loading ? (
                ...
                <>
                  ...
                  <div className="text-xs text-slate-500">
                    {role === "vendor"
                      ? "Tip: POD and COD are enabled by default for vendors. Fill bank details for direct transfers."
                      : "Please ensure your account details are correct to avoid delays in processing your withdrawal."
                    }
                  </div>
                </>
              )}
