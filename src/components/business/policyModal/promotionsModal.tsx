...
export default function PromotionsModal({
  ...
  const defaultOccasions = useMemo<Occasion[]>(() => {
    ...
    const events: Array<[string, number, number]> = [
      ...
      ["Mother's Day", 4, 1], // placeholder; real date computed below (2nd Sunday of May)
      ["Easter Monday", 3, 13], // placeholder (computed below)
      ["International Workers' Day", 4, 1],
      ["Children's Day", 4, 27], // Nigeria: May 27
      ["Democracy Day", 5, 12],
      ["Father's Day", 5, 1], // placeholder; real date computed below (3rd Sunday of June)
      ["Independence Day", 9, 1],
      ["Halloween", 9, 31],
      ["Black Friday", 10, 27], // usually last Fri of Nov - we still add a broader window below
      ["Christmas Eve", 11, 24],
      ["Christmas Day", 11, 25],
      ["Boxing Day", 11, 26],
...
function leaveCampaign(oc: string) {
  ...
  return (
    <div className="fixed inset-0 z-75 flex items-end sm:items-center justify-center" role="dialog" aria-modal="true">
      ...
      <div className="relative w-full max-w-3xl bg-white lg:rounded-2xl rounded-t-2xl shadow-xl p-5 z-10">
