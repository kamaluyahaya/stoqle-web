...
export const RoomItem: React.FC<RoomItemProps> = ({
    ...
    return (
        <motion.div
            ...
            whileTap={{ scale: 0.98 }}
            className={`flex items-center p-3.5 rounded-[1.25rem] cursor-pointer transition-all duration-300 gap-3 group relative ${
                active 
        ...
        >
            <div className="relative flex-shrink-0">
                ...
                >
                    {avatarSrc ? (
                        <img
                            src={avatarSrc}
                            alt={displayName}
                            className={`w-12 h-12 rounded-full object-cover shadow-sm transition-all duration-500 ${
                                active ? "ring-2 ring-red-500 ring-offset-2" : "border-2 border-white"
                            }`}
                        />
                    ) : (
                        <div className={`w-12 h-12 rounded-full flex items-center justify-center font-bold text-lg text-white shadow-md transition-all duration-300 ${
                            active ? "bg-red-500 scale-105" : "bg-slate-200 group-hover:bg-slate-300"
                        }`}>
                ...
                {unread > 0 && (
                    <span className="absolute -top-1 -right-1 bg-red-600 text-white text-[10px] font-black w-5 h-5 flex items-center justify-center rounded-full border-2 border-white ring-2 ring-red-50">
                        {unread}
                    </span>
                )}
            </div>
            ...
            <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between gap-2 mb-0.5">
                    <p className={`font-black text-sm truncate tracking-tight transition-colors ${
                ...
                <div className="flex items-center justify-between gap-2">
                    ...
                    {unread > 0 && !active && (
                       <div className="w-2 h-2 rounded-full bg-red-500 shrink-0 shadow-sm shadow-red-500/50" />
                    )}
                </div>
            </div>

            {active && (
                <div className="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-8 bg-red-500 rounded-r-full shadow-sm" />
            )}
        </motion.div>
    );
};
