...
export default function EditPostModal({ open, post, onClose, onUpdated }: Props) {
  ...
  const handleUpdate = async () => {
    try {
      ...
      onUpdated({ ...post, caption: text, subtitle, privacy, src: localMedia[0]?.url || post.src, allMedia: localMedia });
      onClose();
    } catch (err: any) {
      console.error(err);
    ...
    <div className="fixed inset-0 z-[20000] flex items-end sm:items-center justify-center p-0" role="dialog" aria-modal="true">
      ...
      >
        ...
        <div className="px-4 py-3 flex items-center justify-between relative border-b border-slate-100">
          ...
          <h2 className="text-sm font-bold text-slate-900 absolute left-1/2 -translate-x-1/2 uppercase tracking-wide">Edit Post</h2>
        ...
        <div className="flex-1 overflow-y-auto no-scrollbar">
          <div className="flex flex-col h-full p-6 space-y-8">
            ...
            <div className="w-full relative group">
                        ...
                        <div className="absolute inset-0 pointer-events-none flex flex-col justify-end p-3 pb-6 bg-gradient-to-t from-black/60 via-transparent to-transparent">
                          ...
                          <div className="space-y-2">
                            <div className="flex items-center gap-1.5">
                              ...
                              <button className="px-1.5 py-0.5 bg-red-500 rounded-full text-[6px] font-black text-white ml-0.5 shadow-sm">Follow</button>
                            </div>
                    ...
                    <div className="absolute top-4 right-4 flex gap-2">
                       {mediaList.length > 1 && mediaList[index]?.id && (
                          <button 
                            ...
                            className={`px-2 py-1 backdrop-blur-md rounded text-[10px] items-center gap-1 font-black uppercase flex border transition-all ${coverId === mediaList[index]?.id || (!coverId && index === 0) ? "bg-red-500 text-white border-red-500" : "bg-black/40 text-white border-white/20 hover:bg-black/60"}`}
                          >
                            <CheckIcon className="w-3 h-3" />
                            {coverId === mediaList[index]?.id || (!coverId && index === 0) ? "Cover" : "Set as cover"}
                          </button>
                       )}
                       <div className="px-2 py-1 bg-black/40 backdrop-blur-md rounded text-[10px] text-white font-black uppercase">Image</div>
                    </div>
              ...
              <div className="mt-2 text-[10px] font-bold text-red-500 uppercase text-center px-4">
            ...
            <div className="space-y-6">
              ...
              <div className="pt-2">
                <label className="text-xs font-black text-slate-400 uppercase ml-1 block mb-2">Visibility</label>
                <button
                ...
                >
                  {privacy === "public" && <LockOpenIcon className="w-4 h-4 text-slate-400 group-hover:text-red-500" />}
                  {privacy === "private" && <LockClosedIcon className="w-4 h-4 text-slate-400 group-hover:text-red-500" />}
                  {privacy === "friends" && <UsersIcon className="w-4 h-4 text-slate-400 group-hover:text-red-500" />}
                  <span className="text-[10px] font-black   text-slate-500 group-hover:text-red-600 uppercase">
        ...
        <div className="p-6 bg-white flex gap-3 pb-10 sm:pb-8">
          ...
          <button
            ...
            disabled={loading}
            className="flex-1 py-3.5 rounded-full bg-red-500 text-white font-black text-sm hover:bg-red-600 shadow-xl shadow-red-100 active:scale-95 transition-all disabled:opacity-50 disabled:grayscale flex items-center justify-center gap-2"
          >
            ...
            <>
              ...
              >
                <div className="flex items-center justify-between mb-2">
                  <h3 className="text-xs font-black text-slate-400 uppercase">Visibility</h3>
                ...
                {[
                  ...
                  <button
                    ...
                    className={`w-full flex items-center justify-between p-4 rounded-2xl transition-all ${
                      privacy === opt.id ? "bg-red-50 text-red-600" : "bg-slate-50 text-slate-600 hover:bg-slate-100"
                    }`}
                  >
