  ...
  return createPortal(
    <>
      <div
        role="dialog"
        aria-modal="true"
        className={`fixed inset-0 flex justify-center px-0 py-0 lg:p-2 ${isMobile && isFromReel ? 'items-end' : 'items-center'}`}
      ...
      >
        ...
        <motion.div
          ...
          initial={isMobile ? { x: "100%", opacity: 0, height: (isMobile && isFromReel) ? "80%" : "100%" } : { opacity: 0, scale: 0.3, height: "94vh" }}
          animate={
            isMobile
              ? {
                x: 0,
          ...
          exit={isMobile ? { x: "100%", opacity: 0 } : { opacity: 0, scale: 0.3, height: "94vh" }}
          transition={
            isMobile
              ? (isExpanded
        ...
        >
          ...
          <div
            ...
            role="dialog"
            aria-modal="true"
            className={`relative w-full h-full bg-slate-100 flex flex-col overflow-hidden lg:flex lg:flex-row lg:w-full lg:h-full lg:rounded-2xl shadow-2xl ${isMobile && isFromReel && !isExpanded ? 'rounded-t-[0.5rem]' : ''}`}
            style={{ WebkitOverflowScrolling: "touch" }}
          >
            {/* Main Scrollable Content Area */}
            <div
              ref={modalRef}
              className="flex-1 overflow-y-auto lg:overflow-visible lg:flex lg:flex-row h-full no-scrollbar"
              style={{ WebkitOverflowScrolling: "touch" }}
            >
                            ...
                            return (
                              <div key={p.product_id} className="group cursor-pointer" onClick={() => {
                                ...
                                <div className="aspect-square bg-slate-50 rounded-xl overflow-hidden mb-2 relative border border-slate-50 group-hover:border-rose-100 transition-colors">
                                  {p.first_image ? <Image src={formatUrl(p.first_image)} alt={p.title} fill className="object-cover group-hover:scale-105 transition-transform duration-500" /> : <div className="w-full h-full flex items-center justify-center text-[10px] text-slate-300">No Image</div>}
                                  {p.product_video && (
                          ...
                          {payload.productImages.map((img, index) => {
                            ...
                            return (
                              <div key={index} className="w-full flex justify-center bg-slate-50  overflow-hidden">
                                <Image src={img.url} alt={img.name ?? `${payload.title} image ${index + 1}`} width={800} height={800} className="w-full h-auto object-contain" />
                              </div>
                            );
                          })}
                        </div>
